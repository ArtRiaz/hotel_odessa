...
async def ask_support(message: types.Message):
    text = 'Если вы хотите отправить сообщение, нажмите на кнопку ниже'
...
async def get_support_message(message: types.Message, state: FSMContext):
    ...
    await message.answer('Вы отправили сообщение', reply_markup=get_back())
    await state.reset_state()


def get_support(dp: Dispatcher):
    dp.register_message_handler(ask_support, Text('Задать вопрос'))
